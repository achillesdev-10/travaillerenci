...
/** Résultat du contrôle anti-duplication (POST /api/cz7tk/exams/similarity). */
type SimResult = {
  score: number | null;
  threshold: number;
...
export default function ExamsAdminClient({
  ...
  function redirectToLogin() {
    router.replace('/cz7tk/login?next=/cz7tk/exams');
  }
  ...
  async function handleUpdateStatus(exam: Exam, newStatus: ExamStatus) {
    ...
    try {
      const res = await fetch(`/api/cz7tk/exams/${exam.id}`, {
        method: 'PATCH',
  ...
  async function handleDelete(exam: Exam) {
    ...
    try {
      const res = await fetch(`/api/cz7tk/exams/${exam.id}`, { method: 'DELETE' });
      if (res.status === 401) {
        redirectToLogin();
        return;
      }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        isCreating ? '/api/cz7tk/exams' : `/api/cz7tk/exams/${editingExam!.id}`,
        {
  ...
  async function handleSimilarityCheck(description: string, sourceUrl: string) {
    ...
    setSimResult(null);
    try {
      const res = await fetch('/api/cz7tk/exams/similarity', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
